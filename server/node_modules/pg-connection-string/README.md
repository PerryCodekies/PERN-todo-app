...
var config = parse('postgres://someuser:some***REMOVED***@somehost:381/somedatabase')
```
 ...
 * `postgres://<user>:<***REMOVED***>@<host>:<port>/<database>?<query>` - TCP connection

But see below for more details.

### UNIX Domain Sockets

When user and ***REMOVED*** are not given, the socket path follows `socket:`, as in `socket:/var/run/pgsql`.
This form can be shortened to just a path: `/var/run/pgsql`.

When user and ***REMOVED*** are given, they are included in the typical URL positions, with an empty `host`, as in `socket://user:pass@/var/run/pgsql`.
...
If username and ***REMOVED*** are included, they should be urlencoded.
